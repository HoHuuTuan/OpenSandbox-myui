...
export const sandboxTemplates: SandboxTemplate[] = [
  ...
  },
  {
    ...
    name: "OpenClaw Agent",
    category: "agent",
    description: "OpenClaw gateway broker-first: agent chi di qua Data Broker de lay du lieu da loc va an bot thong tin nhay cam.",
    ...
    envText: [
      ...
      "OPENCLAW_DATA_BROKER_URL=http://host.docker.internal:3302",
      "OPENCLAW_DATA_BROKER_TOKEN=broker-secret",
    ].join("\n"),
    metadataText: "template=openclaw-agent\nproject=agent-lab\ndata-access=broker-only",
    networkPolicyText: [
      "default=deny",
    ...
    recipe: [
      ...
      "Template nay truyen entrypoint /opt/opensandbox/openclaw-entrypoint.sh de phu hop voi lifecycle API hien tai.",
      "Workspace duoc seed san AGENTS, TOOLS va BOOTSTRAP de agent dung broker-query thay vi raw DB hoac raw source.",
      "Network policy mac dinh chi allow Data Broker va mot so domain provider pho bien. Neu dung provider khac, hay them domain tuong ung.",
      "Bam Open de vao Control UI qua proxy cua OpenSandbox. Token gateway mac dinh la 123456.",
    ],
  },
  {
    id: "custom",
